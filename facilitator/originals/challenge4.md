...
**High-level player flow:** `binwalk` → note offsets → `dd` carve → crack `secret.enc` → obtain `qtbl.py` + `STEGO_KEY_386.txt` + `passwords.enc` + `iv.bin` → extract AES key from `nothingtoseehere.jpg` (QTABLEs) using `qtbl.py` + `STEGO_KEY_386.txt` → decrypt `passwords.enc` → use recovered passwords to decrypt `payload.enc` → obtain `flag.txt`.
...
- `STEGO_KEY_386.txt` (hint file included in player bundle; players obtain this via cracking `secret.enc`)
...
- `secret_bundle.zip` (contains: `qtbl.py`, `STEGO_KEY_386.txt`, `passwords.enc`, `iv.bin`)
...
- `keyblock.txt` is creator-only. `STEGO_KEY_386.txt` is included in `secret_bundle.zip` and provided to players after cracking.
...
## - STEGO_KEY_386.txt    (player hint to place in the bundle later)
...
zip -j secret_bundle.zip qtbl.py STEGO_KEY_386.txt passwords.enc iv.bin
...
## 9.2 Extract AES key from the inner stego image using recovered STEGO_KEY_386.txt and player helper
###This needs to be the actual stego key used. The player has STEGO_KEY_386.txt - but they will need to solve to get the same string as we have in keyblock.txt
...
## - Confirm rb/qtbl.py supports 2-LSB + raw-32 extraction and STEGO_KEY_386.txt matches creator's keyblock.txt used during embed
...
## 3) Crack secret.enc (easy) to recover the bundle (qtbl.py, STEGO_KEY_386.txt, passwords.enc, iv.bin)
...
python3 qtbl.py extract -i inner_decoy.jpg -o aeskey.bin -k "$(cat STEGO_KEY_386.txt)"
...
- STEGO_KEY handling: keyblock.txt (creator-only) is used to XOR payload before embedding. Provide STEGO_KEY_386.txt to players inside secret_bundle.zip after weak encryption is cracked.
